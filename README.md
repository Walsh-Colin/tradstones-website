...
The band details come from the supplied Facebook screenshots, and the music section links to `https://www.facebook.com/tradstonemusic/`. No recordings are invented. The gallery displays a placeholder until local photos are supplied. The group photo attached in chat still needs to be saved into `assets/photos/` and added to `photos` in `band-config.js`; it is not included in the local files yet. The dated gig poster is intentionally omitted from this evergreen brochure. The Celtic medallion and sleeve are decorative artwork, not a real release or audio player. Only change the content if the band's information changes.
